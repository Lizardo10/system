...
{	
	...
	function change_mode()
	{
		...
		if ($previous_mode == 'receive' && ($mode =='return' || $mode =='transfer'))
		{
		...
		elseif(($previous_mode =='return' || $previous_mode =='transfer') && $mode =='receive')
		{
	...
	{
		$this->cart->add_paid_store_account_payment_id($receiving_id);
	...
	function toggle_pay_all_store_account()
	{
		...
		if(count($this->cart->get_paid_store_account_ids()) ==  0)
		{
			...
			{
					$this->cart->add_paid_store_account_payment_id($unpaid_recv['receiving_id']);
