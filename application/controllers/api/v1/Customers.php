			...
			if ($person_id === NULL)
      {
      	...
      	$search_field = $this->input->get('search_field');
				...
				if ($search)
				{
					...
					$customers = $this->Customer->search($search,$location_id, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'last_name','asc',$search_field)->result();
					$total_records = $this->Customer->search_count_all($search,$location_id, 0,10000,$search_field);
				}
				else
				{
					$customers = $this->Customer->get_all($location_id,0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'pid','desc')->result();
