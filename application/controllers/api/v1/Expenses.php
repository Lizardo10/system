    ...
    public function index_get($expenses_id = NULL)
    {
			...
			if ($expenses_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$expenses = $this->Expense->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'id','asc',$location_id)->result();
					$total_records = $this->Expense->search_count_all($search, 0,10000,$location_id);
				}
				else
				{
					$expenses = $this->Expense->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'id','desc',$location_id)->result();
