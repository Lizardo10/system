    ...
    public function index_get($price_rules_id = NULL)
    {
			...
			if ($price_rules_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$price_rules = $this->Price_rule->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','asc')->result();
					$total_records = $this->Price_rule->search_count_all($search, 0,10000);
				}
				else
				{
					$price_rules = $this->Price_rule->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','desc')->result();
