			...
			if ($person_id === NULL)
      {
      	...
      	$search_field = $this->input->get('search_field');
				...
				if ($search)
				{
					...
					$suppliers = $this->Supplier->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'last_name','asc',$search_field)->result();
					$total_records = $this->Supplier->search_count_all($search, 0,10000,$search_field);
				}
				else
				{
					$suppliers = $this->Supplier->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'pid','desc')->result();
