...
class Category extends CI_Model
{
	...
	*/
	...
	function save($category_name = "", $hide_from_grid = NULL, $parent_id = NULL, $category_id = FALSE, $category_color = FALSE, $category_image_id = NULL,$system_category = 0)
	{
		if ($category_id == FALSE)
		{
			if ($category_name)
			{
				if($this->db->insert('categories',array('name' => $category_name, 'hide_from_grid' => $hide_from_grid ? 1 : 0, 'parent_id' => $parent_id, 'color' => $category_color ? $category_color : '', 'image_id' => $category_image_id, 'last_modified' => date('Y-m-d H:i:s'),'system_category' => $system_category)))
				{
