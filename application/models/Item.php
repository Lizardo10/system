...
class Item extends CI_Model
{
	...
	*/
	...
	function get_displayable_columns()
	{
		$return  = array(
			...
			'location' => 							array('sort_column' => 'location_at_store', 'label' => lang('items_location_at_store')),
	...
	*/
	function get_item_search_suggestions_without_variations($search,$deleted = 0,$limit=25,$price_field = FALSE)
	{
		...
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
		{
			if ($this->get_custom_field($k)) 
			{
				...
				{
					$this->db->where("FROM_UNIXTIME(custom_field_${k}_value, '%Y-%m-%d') !='1969-12-31' and FROM_UNIXTIME(custom_field_${k}_value, '%Y-%m-%d') = ".$this->db->escape(date('Y-m-d', strtotime($search))), NULL, false);					
				}
				$this->db->limit($limit);
		...
		{
			$this->db->select("items.unit_price,items.cost_price,items.name,item_number,size, item_images.image_id,items.item_id, categories.name as category, MATCH (".$this->db->dbprefix('items').".name) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			$this->db->where("MATCH (".$this->db->dbprefix('items').".name) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);			
			$this->db->where('items.deleted',$deleted);
			$this->db->where('items.system_item',0);
			$this->db->group_by('items.item_id');
			$this->db->limit($limit);
			...
			$this->db->select("items.unit_price,items.cost_price,item_number,items.item_id,item_images.image_id,items.name, categories.name as category, size, MATCH (item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".item_number = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			$this->db->where("MATCH (item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);			
			$this->db->where('items.deleted',$deleted);
			$this->db->where('items.system_item',0);
			$this->db->group_by('items.item_id');
			$this->db->limit($limit);
			...
			$this->db->select("items.unit_price,items.cost_price,item_number,items.item_id,product_id,item_images.image_id,items.name, categories.name as category, size, MATCH (item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".product_id = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
			$this->db->where("MATCH (product_id) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);			
			$this->db->where('items.deleted',$deleted);
			$this->db->where('items.system_item',0);
			$this->db->limit($limit);
			$this->db->order_by('rel DESC, exact_score DESC');
			...
			$this->db->select("items.unit_price,items.cost_price,items.item_id,item_images.image_id,items.name, categories.name as category, size, MATCH (item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel", false);
			$this->db->from('items');
			$this->db->join('item_images', 'items.item_id = item_images.item_id', 'left');
			$this->db->join('categories', 'categories.id = items.category_id','left');
	...
	function get_item_search_suggestions($search,$deleted=0,$price_field = 'unit_price',$limit=25)
	{
		...
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category, MATCH (".$this->db->dbprefix('items').".name) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			$this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
			...
			$this->db->group_start();
			$this->db->where("MATCH (".$this->db->dbprefix('items').".name) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);			
			$this->db->or_where("MATCH (".$this->db->dbprefix('item_variations').".name) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);			
			$this->db->group_end();
			$this->db->limit($limit);
			$this->db->order_by('rel DESC, exact_score DESC');
			...
			$this->db->select("$quantity_field as quantity,item_variations.item_number as item_variation_number,item_variations.deleted as variation_deleted,item_images.image_id,items.*,item_variations.id as item_variation_id, categories.name as category, MATCH (".$this->db->dbprefix('items').".item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".item_number = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			...
			$this->db->where('items.deleted',$deleted);
			$this->db->where('items.system_item',0);
			$this->db->where("MATCH (".$this->db->dbprefix('items').".item_number) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE)", NULL, FALSE);
			...
			$this->db->select("$quantity_field as quantity,item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category,MATCH (product_id) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".product_id = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
	...
	function search($search, $deleted=0,$category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		{					
			if ($this->get_custom_field($k) !== FALSE)
			{
				...
				{							
					$custom_fields[$k]= "FROM_UNIXTIME(".$this->db->dbprefix('items').".custom_field_${k}_value, '%Y-%m-%d') = ".$this->db->escape(date('Y-m-d', strtotime($search)));					
				}
			...
			if ($fields == 'all')
			{
				if ($search)
				{
					...
					{							
						if ($this->config->item('speed_up_search_queries'))
						{	
							$search_main_query = "WHERE ($custom_fields or MATCH (".$this->db->dbprefix('items').".name, ".$this->db->dbprefix('items').".item_number, product_id, description) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") or ".$this->db->dbprefix('items').".item_id = ".$this->db->escape($search).") and ".$this->db->dbprefix('items'). ".deleted=$deleted and system_item = 0";
						}
						else
						{						
			...
			{			
				if ($search)
				{
					...
					else
					{
						...
						{
							//Fulltext
							$search_main_query = "WHERE MATCH($fields) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") and ".$this->db->dbprefix('items').".deleted=$deleted and system_item = 0";												
						}
						else
						{
			...
			if ($fields == 'all')
			{
				if ($search)
				{
					if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
					{
						if ($this->config->item('speed_up_search_queries'))
						{	
							$this->db->where("($custom_fields or MATCH (".$this->db->dbprefix('items').".name, ".$this->db->dbprefix('items').".item_number, product_id, description) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") or ".$this->db->dbprefix('items').".item_id = ".$this->db->escape($search).") and ".$this->db->dbprefix('items'). ".deleted=$deleted and system_item = 0", NULL, FALSE);							
						}
						else
						{						
							$this->db->join('additional_item_numbers', 'additional_item_numbers.item_id = items.item_id', 'left');
							$this->db->where("($custom_fields or MATCH (".$this->db->dbprefix('items').".name, ".$this->db->dbprefix('items').".item_number, product_id, description) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") or ".$this->db->dbprefix('tags').".name LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' ESCAPE '!'"." or ".$this->db->dbprefix('categories').".name LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' ESCAPE '!' or ".$this->db->dbprefix('additional_item_numbers').".item_number LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' ESCAPE '!' or ".$this->db->dbprefix('items').".item_id = ".$this->db->escape($search).") and ".$this->db->dbprefix('items'). ".deleted=$deleted and system_item = 0", NULL, FALSE);		
						}
					}
					else
					{
			...
			{			
				if ($search)
				{
					...
					else
					{
						...
						{
							//Fulltext
							$this->db->where("MATCH($fields) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") and ".$this->db->dbprefix('items').".deleted=$deleted and system_item = 0");												
						}
						else
						{
		...
		if ($fields == 'all')
		{
			if ($search)
			{
				if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
				{
					if ($this->config->item('speed_up_search_queries'))
					{
						$this->db->where("($custom_fields or MATCH (".$this->db->dbprefix('items').".name, ".$this->db->dbprefix('items').".item_number, product_id, description) AGAINST (".single_quote_string(escape_full_text_boolean_search($search).(substr($search,-1) != '"' && substr($search,-1) != ' ' ? '*' : ''))." IN BOOLEAN MODE".") and ".$this->db->dbprefix('items'). ".deleted=$deleted and system_item = 0", NULL, FALSE);							
					}
					else
					{
						$this->db->join('additional_item_numbers', 'additional_item_numbers.item_id = items.item_id', 'left');
	...
	*/
	function get_sale_price(array $params)
	{
		...
		{
			return to_currency_no_money($item_location_tier_row->unit_price, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_location_tier_row) && $item_location_tier_row->percent_off)
		{
			if ($variation_info && $variation_info->unit_price)
			{
				$item_unit_price = $variation_info->unit_price;
			}
			else
			{
				$item_unit_price = $item_location_info->unit_price ? $item_location_info->unit_price : $item_info->unit_price;
			}
			return to_currency_no_money($item_unit_price *(1-($item_location_tier_row->percent_off/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_location_tier_row) && $item_location_tier_row->cost_plus_percent)
		{
			...
			return to_currency_no_money($item_cost_price *(1+($item_location_tier_row->cost_plus_percent/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_location_tier_row) && $item_location_tier_row->cost_plus_fixed_amount)
		{
			...
			return to_currency_no_money($item_cost_price + $item_location_tier_row->cost_plus_fixed_amount, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_tier_row) && $item_tier_row->unit_price)
		{
			return to_currency_no_money($item_tier_row->unit_price, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_tier_row) && $item_tier_row->percent_off)
		{
			...
			return to_currency_no_money($item_unit_price *(1-($item_tier_row->percent_off/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_tier_row) && $item_tier_row->cost_plus_percent)
		{
			...
			return to_currency_no_money($item_cost_price *(1+($item_tier_row->cost_plus_percent/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif (!empty($item_tier_row) && $item_tier_row->cost_plus_fixed_amount)
		{
			...
			return to_currency_no_money($item_cost_price + $item_tier_row->cost_plus_fixed_amount, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif($tier_info->default_percent_off)
		{
			...
			return to_currency_no_money($item_unit_price *(1-($tier_info->default_percent_off/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif($tier_info->default_cost_plus_percent)
		{
			...
			return to_currency_no_money($item_cost_price *(1+($tier_info->default_cost_plus_percent/100)), $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif($tier_info->default_cost_plus_fixed_amount)
		{
			...
			return to_currency_no_money($item_cost_price + $tier_info->default_cost_plus_fixed_amount, $this->config->item('round_tier_prices_to_2_decimals') ? 2 : 10);
		}
		elseif($variation_id && $variation_info->unit_price)
		{
			...
			{
				return to_currency_no_money($variation_info->promo_price, 10);
			}
			...
			return to_currency_no_money($variation_info->unit_price, 10);
		}
		else
		{
			$today =  strtotime(date('Y-m-d'));
