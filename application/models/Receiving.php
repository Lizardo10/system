...
class Receiving extends CI_Model
{
	...
	function save ($cart)
	{
		...
		$paid_recv = $cart->get_paid_store_account_ids();
		if (!empty($paid_recv))
		{
		...
		foreach($items as $line=>$item)
		{
			...
			{				
				...
				if ($item->variation_id)
				{
					$item_loc_var_to_save_qty = $this->Item_variation_location->get_location_quantity($item->variation_id,$location_id) + ($item->quantity * -1);
	...
	function delete($receiving_id, $all_data = false, $update_quantity = true)
	{
	  ...
	  if($recv_info['store_account_payment'])
		...
		if ($update_quantity)
		{
			$this->db->select('receivings.location_id, item_id, quantity_purchased, quantity_received, transfer_to_location_id,item_variation_id');
