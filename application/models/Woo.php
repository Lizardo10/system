...
{	
	...
	function get_tags($use_cache = TRUE)
	{
		...
		$tags = $woo_tags->get_tags();
 		...
 		{
 			if (!isset($phppos_tags[strtoupper($tag['name'])]))
 			{
 				$phppos_tag_id = $this->Tag->save(ucwords(strtolower($tag['name'])));
 			} else {
 				$phppos_tag_id = $phppos_tags[strtoupper($tag['name'])];
 			}
	...
	function get_categories($use_cache = TRUE)
	{
		...
		{		
			...
			if($image_url && (!isset($categories_indexed_by_name[$cat_path]) && (!$categories_indexed_by_name[$cat_path]['image_id'])))
			{
		    $allowed_extensions = array('png', 'jpg', 'jpeg', 'gif');
				$extension = strtolower(pathinfo(basename($image_url), PATHINFO_EXTENSION));
			...
			if($image_file_id)
			{
				$this->Category->link_image($categories_indexed_by_name[strtoupper($cat_path)], $image_file_id);
			}
	...
	function sync_inventory_changes()
	{
		...
		{
			$this->import_ecommerce_items_into_phppos(TRUE);
		}
		
		if (!in_array('export_phppos_items_to_ecommerce', $ecommerce_cron_sync_operations))
		{
			//Update inventory values
			$Woo_products = new Woo_products($this);
			return $Woo_products->batch_products(array(),array('update'));
		}
	...
	function import_ecommerce_items_into_phppos($inventory_only = FALSE)
	{
		...
		{				
			...
			if($item_row)
			{
				$this->update_inventory_from_ecommerce_to_phppos($product, $item_row);
			}
			...
			if($product['type'] == 'variable')
			{
				...
				foreach($variations as $variation)
				{
					...
					if($item_row && $variation_row)
					{
						$this->update_item_variation_inventory_from_ecommerce_to_phppos($item_row, $variation, $variation_row);
					}
				}
			}
		}
	...
	private function add_update_item_variation_from_ecommerce_to_phppos($item_row, $woo_variation, $variation_row = false)
	{
		...
		$item_variation = array(
			...
			'unit_price' => $woo_variation['regular_price'] ? $woo_variation['regular_price'] : null,
		...
		if ($variation_id && isset($woo_variation['image']['id']) && $woo_variation['image']['id'])
		{
			...
			{
		    $allowed_extensions = array('png', 'jpg', 'jpeg', 'gif');
				$extension = strtolower(pathinfo(basename($woo_variation['image']['src']), PATHINFO_EXTENSION));
	...
	private function update_item_variation_inventory_from_ecommerce_to_phppos($item_row, $woo_variation, $variation_row)
	{
		$variation_id = $variation_row['id'];
		
		$product_variation_id = $woo_variation['id'];
		
		$previous_woo_product_variation_quantity = 0;
		$current_pos_variation_quantity = 0;
		$ecommerce_last_modified = getDateFromGMT($woo_variation['date_modified_gmt']);
		
		$current_pos_variation_quantity = $this->get_item_variation_quantity($variation_id);
		
		if ($current_pos_variation_quantity === NULL)
		{
			$actual_quantity = $woo_variation['stock_quantity'];
		}
		else
		{
			$previous_woo_product_variation_quantity = $variation_row['ecommerce_variation_quantity'];
			$woo_quantity_diff = $woo_variation['stock_quantity'] - $previous_woo_product_variation_quantity;		
			$pos_quantity_diff =  $current_pos_variation_quantity - $previous_woo_product_variation_quantity;				
			$actual_quantity = $previous_woo_product_variation_quantity + ($woo_quantity_diff + $pos_quantity_diff);
		}
				
		if($actual_quantity != $current_pos_variation_quantity)
		{
			$this->log('update_item_variation_inventory_from_ecommerce_to_phppos');
			
			$item_variation_data = array(
				'ecommerce_variation_id'=> $woo_variation['id'], 
				'ecommerce_variation_quantity'=> $woo_variation['stock_quantity'],
				'ecommerce_last_modified' => $ecommerce_last_modified,
				'last_modified' => $ecommerce_last_modified,
			);
		
			$this->Item_variations->save($item_variation_data, $variation_id);
		
			$cron_job_entry=lang('woo_cron_job_entry');
		
			if($actual_quantity - $current_pos_variation_quantity !== 0)
			{
				$inventory_data = array(
					'trans_items' => $item_row['item_id'],
					'item_variation_id'=>$variation_id,
					'trans_user'=>1,
					'trans_comment'=>$cron_job_entry,
					'trans_inventory'=> ($actual_quantity - $current_pos_variation_quantity),
					'location_id'=>$this->ecommerce_store_location,
					'trans_current_quantity' => $actual_quantity,
			...
			if($actual_quantity !=  $woo_variation['stock_quantity'])
			{
				$data = array('stock_quantity' => (int)$actual_quantity);
				$woo_product_variations	=	new Woo_product_variations($this);
				$products = $woo_product_variations->update_product_variation($variation_id, $data);
			}
		}
	}
	
	private function update_inventory_from_ecommerce_to_phppos($woo_product, $item_row)
	{		
		$item_id = $item_row['item_id'];
		$product_id = $woo_product['id'];
		
		$previous_woo_product_quantity = 0;
		$current_pos_quantity = 0;
		$ecommerce_last_modified = getDateFromGMT($woo_product['date_modified_gmt']);
		
		$current_pos_quantity = $this->get_item_quantity($item_id) !== NULL ? floor($this->get_item_quantity($item_id)) : NULL;
		
		if ($current_pos_quantity === NULL)
		{
			$actual_quantity = $woo_product['stock_quantity'];
		}
		else
		{
			$previous_woo_product_quantity = $item_row['ecommerce_product_quantity'];
			$woo_quantity_diff = $woo_product['stock_quantity'] - $previous_woo_product_quantity;		
			$pos_quantity_diff =  $current_pos_quantity - $previous_woo_product_quantity;				
			$actual_quantity = $previous_woo_product_quantity + ($woo_quantity_diff + $pos_quantity_diff);
		}
		
		if($actual_quantity != $current_pos_quantity)
		{
			
			$this->log('update_inventory_from_ecommerce_to_phppos: '. $item_row['item_id']);
			
			$item_array = array(
				'ecommerce_product_id'=>$product_id, 
				'ecommerce_product_quantity'=> $woo_product['stock_quantity'],
				'ecommerce_last_modified' => $ecommerce_last_modified,
				'last_modified' => $ecommerce_last_modified,
			);
			...
			$inventory_array = array(
				...
				'trans_user'=>1,
				'trans_comment'=>$cron_job_entry,
				'trans_inventory'=> ($actual_quantity - $current_pos_quantity),
				'location_id'=>$this->ecommerce_store_location,
				'trans_current_quantity' => $actual_quantity,
			);
			...
			if($actual_quantity !=  $woo_product['stock_quantity'])
			{
				$woo_products	=	new Woo_products($this);
				$products = $woo_products->update_product($item_id, array('stock_quantity' => (int)$actual_quantity));
			}
		}
	}
	...
	{						
		...
		$item_array = array(
			'name'=>$product_name,
			'description' => strip_tags($product_short_description),
			'long_description' => strip_tags($product_description),
		...
		foreach($all_custom_attrs_for_item as $key => $attr)
		{
			...
			{	
				if (strtoupper($attr['name']) == strtoupper($woo_attr['name']))
				{
					$found = TRUE;
					continue;
				}
		...
		if(count($product_attributes) > 0)
		{
			...
			{
				if($product_attribute['id'] && isset($phppos_attributes[strtoupper($product_attribute['name'])][0]))
				{
					//global
					$pos_attribute = $phppos_attributes[strtoupper($product_attribute['name'])][0];
					$attribute_ids_to_save[] = $pos_attribute['id'];
				}
				else
				{
					if(isset($phppos_attributes[strtoupper($product_attribute['name'])][$item_id]))
					{
						//existing custom
						$pos_attribute = $phppos_attributes[strtoupper($product_attribute['name'])][$item_id];
						$attribute_ids_to_save[] = $pos_attribute['id'];
					}
					else
					{
					...
					{
						if(!isset($pos_attribute['terms'][strtoupper($option)]))
						{				
							$id = $this->Item_attribute_value->save($option,$pos_attribute['id']);
							$pos_attribute['terms'][strtoupper($option)] = array('id' => $id);
						}
					}
				}
				...
				foreach($product_attribute['options'] as $option)
				{
					$attribute_value_ids_to_save[] = $pos_attribute['terms'][strtoupper($option)]['id'];
				}
		...
		if (isset($woo_product['images'][0]) && $woo_product['images'][0]['id'])
		{
			foreach($woo_product['images'] as $woo_image)
			{
				...
				{
			    $allowed_extensions = array('png', 'jpg', 'jpeg', 'gif');
					$extension = strtolower(pathinfo(basename($woo_image['src']), PATHINFO_EXTENSION));
	...
	function get_attributes($use_cache = TRUE)
	{
		...
		{
			$phppos_attributes[strtoupper($phppos_attribute['name'])] = $phppos_attribute['id'];
		}
		...
		foreach($attributes as $woo_attr)
		{
			...
			if (!isset($phppos_attributes[strtoupper($woo_attr['name'])]))
			{
				$item_attr_data = array('name' => ucwords(strtolower($woo_attr['name'])));
				$this->Item_attribute->save($item_attr_data);
				$phppos_attribute_id = $item_attr_data['id'];
			}
			else
			{
				$phppos_attr_id = $phppos_attributes[strtoupper($woo_attr['name'])];
	...
	{	
		...
		{
			$phppos_attribute_values[$woo_attribute_id.'|'.strtoupper($phppos_attribute_value['name'])] = $phppos_attribute_value['id'];
		}
		...
		foreach($attribute_terms as $attribute_term)
		{
			...
			if (isset($phppos_attribute_values[$woo_attribute_id.'|'.strtoupper($attribute_term['name'])]))
			{
				$phppos_attribute_value_id = $phppos_attribute_values[$woo_attribute_id.'|'.strtoupper($attribute_term['name'])];
	...
	function get_tax_classes($use_cache = TRUE)
	{
		...
		$tax_classes = $woo_tax_classes->get_tax_classes();
 		...
 		{
 			$phppos_tax_classes[strtoupper($phppos_tax_class['name'])] = $phppos_tax_class_id;
 			if($phppos_tax_class['ecommerce_tax_class_id'])
 			{
 				if(!in_array($phppos_tax_class['ecommerce_tax_class_id'], array_column($tax_classes, 'slug')))
 				{
 					$this->Tax_class->delete($phppos_tax_class_id);
 				}
 			}
 		}
 		...
 		{
 			if (!isset($phppos_tax_classes[strtoupper($tax_class['name'])]))
 			{
				$tax_class_data = array('name' => $tax_class['name']);
				$this->Tax_class->save($tax_class_data);
 				$phppos_tax_class_id = $tax_class_data['id'];
 			} else {
 				$phppos_tax_class_id = $phppos_tax_classes[strtoupper($tax_class['name'])];
 			}
			
 			$this->link_tax_class($phppos_tax_class_id, $tax_class['slug']);
			
 			$return_woo_tax_classes[strtoupper(html_entity_decode($tax_class['name']))] =  $tax_class['slug'];
 		}
