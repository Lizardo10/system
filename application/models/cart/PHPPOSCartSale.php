...
{		
	...
	public static function get_instance_from_sale_id($sale_id,$cart_id = NULL,$is_editing_previous = FALSE)
	{
		...
		$paid_store_account_ids = $CI->Sale->get_store_accounts_paid_sales($sale_id);
	...
	*/
	function determine_new_prices_for_tier_change()
	{
		...
		foreach ($items as $line=>$item )
		{
			if (get_class($item) == 'PHPPOSCartItemSale')
			{
				...
				if((isset($item_variation_info) && $price == $item_variation_info->unit_price) || $price==$item_info->unit_price || $price == $item_location_info->unit_price || (($price == $previous_price) && (($price !=0 && $previous_price!=0) || $this->previous_tier_id)))
				{	
	...
	function discount_all($percent_discount)
	{
		...
		foreach($items as $index=>$item)
		{
			...
			{
				if($item->max_discount_percent !== NULL && floatval($percent_discount) > floatval($item->max_discount_percent))
				{
					return false;
				}
			}
		...
		if(!$is_edit && !$is_delete && $r == $items_to_buy && ($max == 0 || ($discount_item_quantity != $max)))
		{
			if(!$CI->config->item('disable_price_rules_dialog'))
			{
				if ($rule['type'] != 'simple_discount')
				{
					...
					$CI->view_data['number_to_add'] = $is_bogo_rule ? $rule['items_to_get'] : 1;
				}
			}
		}
	...
	function process_barcode_scan($barcode_scan_data,$options = array())
	{
		...
		{
			$quantity = $mode=="sale" || $mode == 'estimate' ? 1:-1;
		}
		...
		elseif($this->is_valid_item_kit($barcode_scan_data))
		{
			...
			{
				$item_kit_to_add->quantity = $item_kit_to_add->default_quantity;
			}
		...
		elseif($this->is_valid_item($barcode_scan_data))
		{
			...
			{
				$item_to_add->quantity = $item_to_add->default_quantity;
			}
