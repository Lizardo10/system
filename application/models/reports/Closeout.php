...
class Closeout extends Report
{
	...
	public function getOutputData()
	{
		...
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'receiving_type' => 'all', 'offset'=> 0, 'export_excel' => 0));
		
		$this->db->select('receiving_id, SUM(total) as total', false);
		$this->db->from($this->db->dbprefix('receivings'));
		$this->db->where('suspended!=2');
		$this->db->where('deleted', 0);
		...
		$receivings_totals_for_payments = array();
