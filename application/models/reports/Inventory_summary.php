...
class Inventory_summary extends Report
{
	...
	public function getSummaryData()
	{
		...
		$this->db->select($full_sum_query.' as total_items_in_inventory, SUM(COALESCE('.$this->db->dbprefix('item_variations').'.cost_price,'.$this->db->dbprefix('location_items').'.cost_price, '.$this->db->dbprefix('items').'.cost_price) * ('.$quantity_query.')) as inventory_total,
		SUM(COALESCE('.$this->db->dbprefix('item_variations').'.cost_price,'.$this->db->dbprefix('location_items').'.cost_price, '.$this->db->dbprefix('items').'.cost_price) * ('.$quantity_query.')) / '.$full_sum_query.' as weighted_cost,
		SUM(COALESCE('.$this->db->dbprefix('item_variations').'.unit_price,'.$this->db->dbprefix('location_items').'.unit_price, '.$this->db->dbprefix('items').'.unit_price) * ('.$quantity_query.')) as inventory_sale_total', FALSE);
		$this->db->from('items');
