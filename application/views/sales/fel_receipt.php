		...
		<div class="col-md-6">
			<div class="hidden-print search no-left-border">
				<ul class="list-inline print-buttons">
					...
					?>
						<li><?php $code = $response[ResponseDATA2]; ?>
							<button class="btn btn-primary btn-lg hidden-print" id="fufillment_sheet_button" onclick="window.open('<?php echo site_url("sales/view_invoice/"); ?>', '_blank');"> <?php echo "version HTML"; ?></button>
						</li>
					<?php } ?>
		...
		<div class="panel panel-piluku">
			<div class="panel-body panel-pad">
						...
						<ul class="list-unstyled invoice-detail" style="margin-bottom:2px;">
							<li>
								<?php if ($receipt_title) { ?>
									<?php echo H("DOCUMENTO TRIBUTARIO ELECTRONICO"); ?><?php echo ($total) < 0 ? ' (' . lang('sales_return') . ')' : ''; ?>
									<br>
								<?php } ?>
								<strong><?php echo H($transaction_time) ?></strong>
							</li>

							<li><span><?php echo "REF:"; ?></span><?php echo H($sale_id); ?></li>
				...
				<div class="invoice-table">
					<div class="row">
						...
						<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?> gift_receipt_element">
							<div class="invoice-head text-right item-price"><?php echo lang('common_price') . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . lang('common_tax') : ''); ?></div>
						</div>
						...
						<?php } ?>
						<div class="col-md-<?php echo $xs_col; ?> col-sm-<?php echo $xs_col; ?> col-xs-<?php echo $xs_col; ?>">
							<div class="invoice-head pull-right item-total gift_receipt_element"><?php echo lang('common_total') . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . lang('common_tax') : ''); ?></div>
						</div>
				...
				$cert_data = array(
					...
					'custom_field_1_value' => $response[Autorizacion],
					'custom_field_2_value' => $response[Serie],
					'custom_field_3_value' => $response[NUMERO],
					'custom_field_4_value' => $response[Fecha_DTE]
				);

				$this->Sale->update($cert_data, $sale_id_raw);
				...
				<div class="row">
					<div class="col-md-12 col-sm-12 col-xs-12">
						...
						<div class="invoice-policy" id="invoice-policy-return-mobile" style="display: none;">
...
<?php echo lang('common_item'); ?>            <?php echo lang('common_price'); ?> <?php echo lang('common_quantity'); ?><?php if ($discount_exists) {
...
<?php echo (isset($show_payment_times) && $show_payment_times) ?  date(get_date_format() . ' ' . get_time_format(), strtotime($payment->payment_date)) : lang('common_payment'); ?>  <?php if (($is_integrated_credit_sale || sale_has_partial_credit_card_payment($cart) || sale_has_partial_ebt_payment($cart)) && ($payment->payment_type == lang('common_credit') ||  $payment->payment_type == lang('sales_partial_credit') || $payment->payment_type == lang('common_ebt') || $payment->payment_type == lang('common_partial_ebt') ||  $payment->payment_type == lang('common_ebt_cash') ||  $payment->payment_type == lang('common_partial_ebt_cash'))) {
																																																																																												echo $payment->card_issuer . ': ' . $payment->truncated_card; ?> <?php } else { ?><?php $splitpayment = explode(':', $payment->payment_type);
																																																																																																																																					echo $splitpayment[0]; ?> <?php } ?><?php echo $this->config->item('round_cash_on_sales') && $payment->payment_type == lang('common_cash') ?  str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency(round_to_nearest_05($payment->payment_amount))) : str_replace('<span style="white-space:nowrap;">-</span>', '-', to_currency($payment->payment_amount)); ?>
